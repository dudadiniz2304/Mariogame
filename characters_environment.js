...
// Modifique o status do jogo se apertar o botão jogar
function changeGameStatud(character){
 if(narizx !="" && gameConfig.status==="start" && GameStatus=="start") {
   ...
   //tocar a musica do mundo do mario
   world_start.play();
...
function getCoins(coin,character){
  if( character.overlap(coin) && character.live && coin.get==false){
    character.coins+=1;
    coin.get=true;
    //tocar a musica de quando o mário pega a moeda
    mario_coin.play();
...
function jumping(character){
  if( (narizy < 168  &&character.live) || (touchIsDown&&character.live) ){
    character.velocity.y+=gameConfig.jump;
    // tocar a musica de quando o mário pula
    mario_jump.play();
  }
}
...
function die(character){
    ...
    {
// tocar a musica de quando o mário morre
mario_die.play();
    }
}
...
//tocar a musica de jogo perdido
mario_gameover.play();
  }
